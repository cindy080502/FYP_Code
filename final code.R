...
acf <- acf(flca$kt,lag.max=50,cex.lab = 1.5)
#plot(acf)
pacf <- pacf(flca$kt,lag.max=50,cex.lab = 1.5) 

acf <- acf(mlca$kt,lag.max=50,cex.lab = 1.5)
#plot(acf)
pacf <- pacf(mlca$kt,lag.max=50,cex.lab = 1.5) 
...
marima<-auto.arima(mlca$kt,ic="aic")
